# main pipeline for PathoTracker (NGS)
pwd
date
echo ${1}
isolate=${1}
...
cat ../../result/${isolate}/${isolate}-select-cluster-identity_all.txt | 
	while read cluster ; do 
		echo ${isolate}
		echo ${cluster}
		python3 ../../pipline/get_best_isolate.py ${isolate} cluster${cluster}
		if ((i>4)); then
                break
        	fi
        	((i=i+1))
...
python3 ../../pipline/select_db_isolate-93cluster_average-NGS.py ${isolate}
